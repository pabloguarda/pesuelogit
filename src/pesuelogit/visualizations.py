...
def plot_predictive_performance(train_losses: pd.DataFrame,
                                ...
                                **kwargs):
    # fig, ax = plt.subplots(figsize = (5,4))
    if epochs_end_learning_stage is None:
        fig, ax = plt.subplots(figsize=(5, 4))
    else:
    ...
    if 'travel time' in curves:
        ...
        patches.append(mpatches.Patch(color='blue', label='Link flows'))

        ax.plot(train_losses['epoch'], train_losses[prefix_metric + '_tt'], label="Travel times", color='red',
                linestyle='-')

        if show_validation:
            ax.plot(val_losses['epoch'], val_losses[prefix_metric + '_tt'], label="Travel times (val)", color='red',
                    linestyle='--')

    if 'link flow' in curves:

        patches.append(mpatches.Patch(color='red', label='Travel times'))

        ax.plot(train_losses['epoch'], train_losses[prefix_metric + '_flow'], label="Link flows", color='blue',
                linestyle='-')

        if show_validation:
            ax.plot(val_losses['epoch'], val_losses[prefix_metric + '_flow'], label="Link flows (val)", color='blue',
                    linestyle='--')

    if 'equilibrium' in curves and prefix_metric + '_eq_flow' in train_losses.columns:

        patches.append(mpatches.Patch(color='gray', label='Equilibrium'))

        ax.plot(train_losses['epoch'], train_losses[prefix_metric + '_eq_flow'], label="Equilibrium", color='gray',
                linestyle='-')

        if show_validation:
            ax.plot(val_losses['epoch'], val_losses[prefix_metric + '_eq_flow'], label="Equilibrium (val)",
                    color='gray',
                    linestyle='--')
