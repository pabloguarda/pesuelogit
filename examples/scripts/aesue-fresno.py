...
# df.loc[(df['tt_ff'] == 0 ) & (df['link_type'] == 'LWRLK'),'tt_ff']
...
# (TODO: enable normalization in get_design_tensor method. See if tensorflow have it)
df[features_Z + ['tt_avg'] + ['tt_ff']] \
    = preprocessing.MinMaxScaler().fit_transform(df[features_Z + ['tt_avg'] + ['tt_ff']])

# df['tt_ff'] = preprocessing.MinMaxScaler().fit_transform(df[['tt_ff']])
